...
const CardProduct = ({ product }: Props) => {
	...
	const handleClick = async (product: Product) => {
		...
		toast({
			...
			render: () => <AlertRedeem product={product} result={result} />,
		});
	};
			...
			>
				{user.points >= product.cost ? (
					...
					<Text
						...
						color='white'
					>
						you need {product.cost} <Img src='./assets/coin.svg' />
					</Text>
				)}
				<Box>
					<Box
					...
					<Box p='6'>
						<Text textAlign='left' color='grayCustom'>
							{product.category}
						</Text>
				...
				<Box
					className={
						user.points >= product.cost ? 'content-hover' : 'content-hover-none'
					}
				>
					<Img
						position='absolute'
					...
					<Box width='90%' className='content-info'>
						<Text
							...
							fontSize={35}
						>
							{product.cost} <Img src='./assets/coin.svg' paddingTop='5px' />
						</Text>
						<Button
							background='white'
							borderRadius={30}
